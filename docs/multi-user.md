...
- **Guest memory** (the in-memory store). Re-seeded lazily on each guest's next turn. See `docs/memory-setup.md`.
- **Session history** (`InMemorySessionStore`). Rebuilt from empty on restart. See the docstring at the top of `store.ts`.
- **Cached agents.** LRU-bounded at `WEB_APP_MAX_CACHED_GUEST_AGENTS` (default 64). The oldest cached agent is evicted under load; that guest gets a fresh build on their next turn (but their memory survives in the shared in-memory store for the lifetime of the process).

## Follow-ups tracked in beads

- `DeepAgentTemplate-ykgj` — Durable session store (Redis/DB) for cross-restart / cross-instance continuity.
- `DeepAgentTemplate-58p3` — S3 memory store adapter (durable guest memory).
