...
// Async event queue shared between the inline executor and the worker. Pushed
// events are consumed via the async iterator (used inline) or relayed into a
// Redis Stream (worker path).
...
// The shared turn-runner: re-establishes the trace context from the W3C
// carrier, opens the `agent_runtime.execute_turn` span, acquires the agent,
// streams UI updates into the queue, and resolves the terminal outcome. Used
// by the BullMQ worker (cross-process).
export class TurnRunner {
  ...
  // Begin a turn under a fresh turn span. Returns the queue + result promise;
  // the caller decides whether to relay events inline or via Redis Streams.
  start(
