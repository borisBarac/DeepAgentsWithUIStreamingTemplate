...
describe("createRedisKeys", () => {
  it("namespaces every key under the configured prefix", () => {
    ...
    expect(keys.workflowState("thread-id-1")).toBe("dat:workflow:thread-id-1");
  });
...
describe("identityMaterial + hashNamespace", () => {
  it("joins identity components with a NUL byte so prefixes cannot collide", () => {
    ...
    // Distinct tuples that would otherwise share a concatenation must differ.
...
describe("TTL config", () => {
  it("returns documented defaults", () => {
    ...
    expect(DEFAULT_TTL_SECONDS.workflowState).toBeGreaterThan(DEFAULT_TTL_SECONDS.session);
  ...
  it("merges overrides on top of defaults", () => {
    ...
    // Untouched keys inherit the defaults.
    expect(resolved.session).toBe(DEFAULT_TTL_SECONDS.session);
    expect(resolved.workflowState).toBe(DEFAULT_TTL_SECONDS.workflowState);
  });
});
