...
describe("RedisSessionLock", () => {
  ...
  it("refreshes the lease only while our fencing token still matches", async () => {
    ...
    // A stale token (different from the holder's) must not refresh.
  ...
  it("releases the lock only when the stored fencing token matches", async () => {
    ...
    // Stale token: release returns true (no-op) but the lock stays put.
    ...
    // After release, a new run can acquire immediately.
  ...
  it("allocates a token greater than every earlier token after lease expiry", async () => {
    // Fencing counters have no TTL — they remain monotonic for the Redis
    // keyspace lifetime. A re-acquire after lease expiry must produce a
    // strictly greater token than every earlier one.
    const lock = new RedisSessionLock({ client: client.asRedis(), keyPrefix: "dat:" });
    const tokens: number[] = [];

    // Acquire, release, re-acquire several times.
    for (let i = 0; i < 3; i++) {
    ...
    // Simulate lease expiry: delete the lock key WITHOUT touching the counter.
    const lockKey = createRedisKeys("dat:").sessionLock(TENANT, USER, SESSION);
    await client.del(lockKey);

    // Re-acquire after "expiry" — the new token must be strictly greater than
    // every earlier token.
