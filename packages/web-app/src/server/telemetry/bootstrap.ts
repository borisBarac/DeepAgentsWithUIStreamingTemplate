...
// Registered once per process. When no exporter/reader is configured, the
// global providers are still wired (so the OpenTelemetry API is usable and
// spans/metrics are recorded as no-ops) but nothing is exported. Execution is
...
export function registerTelemetry(options: TelemetryExporterOptions = {}): void {
  ...
  // Connect your OpenTelemetry Collector exporter here.
...
export async function shutdownTelemetry(): Promise<void> {
  ...
  // Reset OTel globals so a fresh registration (e.g. across test files) can
