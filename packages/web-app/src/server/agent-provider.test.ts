...
import type { LinkloomResearchConnection } from "@deep-agent-template/core";
import {
...
// Minimal structural stand-in for a connected Linkloom MCP tool. The built
// agent only needs the tool shape that deepagents forwards to the researcher.
function fakeResearchTool(name: string) {
  return {
    name,
    invoke: async () => "ok",
  } as unknown as LinkloomResearchConnection["tools"][number];
}

function fakeLinkloomConnection(
  tools: LinkloomResearchConnection["tools"],
): LinkloomResearchConnection {
  const close = mock(async () => {});
  return {
    close,
    tools,
    client: {} as LinkloomResearchConnection["client"],
    [Symbol.asyncDispose]: close,
  } as LinkloomResearchConnection;
}

describe("createAgentForIdentity — Linkloom researcher tools", () => {
  beforeEach(() => __resetGuestMemoryForTest());
  afterEach(() => __resetGuestMemoryForTest());

  it("builds gracefully without Linkloom tools (degraded mode)", async () => {
    await withTempMemory({}, async () => {
      // __resetGuestMemoryForTest pins the connection to degraded (null), so
      // no network call is made and the agent still builds.
      const agent = await createAgentForIdentity({ tenantId: "t", userId: "alice" });
      expect(agent).toBeTruthy();
    });
  });

  it("builds successfully when Linkloom tools are available", async () => {
    await withTempMemory({}, async () => {
      __setLinkloomConnectionForTest(
        fakeLinkloomConnection([fakeResearchTool("scrape"), fakeResearchTool("html_to_markdown")]),
      );
      const agent = await createAgentForIdentity({ tenantId: "t", userId: "alice" });
      expect(agent).toBeTruthy();
    });
...
describe("disposeLinkloomConnection", () => {
  beforeEach(() => __resetGuestMemoryForTest());
  afterEach(() => __resetGuestMemoryForTest());

  it("closes the resolved shared connection (graceful worker shutdown)", async () => {
    const connection = fakeLinkloomConnection([fakeResearchTool("scrape")]);
    __setLinkloomConnectionForTest(connection);
  ...
  it("is a no-op when no connection was ever resolved", async () => {
    __setLinkloomConnectionForTest(undefined); // restore lazy resolution
    await expect(disposeLinkloomConnection()).resolves.toBeUndefined();
  });

  it("swallows close failures during shutdown", async () => {
    const failing = {
      close: mock(async () => {
    ...
    } as unknown as LinkloomResearchConnection;
    __setLinkloomConnectionForTest(failing);
