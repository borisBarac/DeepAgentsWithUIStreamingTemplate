...
function createPersistence(store: WorkflowStateStore) {
  const persist = (runtime: unknown, state: WorkflowState): void => {
    store.save(threadId(runtime), state);
  };

  const archive = (runtime: unknown, state: WorkflowState): void => {
    store.archive(threadId(runtime), state);
  };

  const load = (runtime: unknown): WorkflowState | undefined => store.load(threadId(runtime));

  return { persist, archive, load };
}

type PersistFn = (runtime: unknown, state: WorkflowState) => void;
type LoadFn = (runtime: unknown) => WorkflowState | undefined;
type ArchiveFn = (runtime: unknown, state: WorkflowState) => void;
...
function runBeforeAgent(
  ...
  if (!state) {
    ...
    deps.archive(runtime, state);
    state = bootstrapWorkflowState(messages, latestUserMessage, deps.productGenerationEnabled);
  }
  deps.persist(runtime, state);
}
...
): { messages: HumanMessage[]; jumpTo: "model" } | undefined {
  const state = deps.load(runtime);
...
function handleClarificationSubmission(
  request: ToolCallRequest,
  state: WorkflowState,
...
function createHasWorkflowState(store: WorkflowStateStore) {
  return (id: string): boolean => store.load(id) !== undefined;
}
...
function createTickControllerBudget({
  ...
  return <S extends Record<string, unknown>, C>(
    state: WorkflowState,
    feedbackText: string,
    request: ToolCallRequest<S, C>,
...
export function createWorkflowControllerMiddleware(options: WorkflowControllerOptions) {
  ...
  const middleware = createMiddleware({
    ...
    beforeAgent: async (agentState, runtime) => {
      runBeforeAgent(agentState.messages, runtime, {
        load,
        archive,
        persist,
    ...
    wrapModelCall: async (request, handler) => {
      const state = load(request.runtime);
