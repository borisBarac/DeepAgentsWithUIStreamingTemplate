...
2. As a core maintainer, I want the test to assert that the workflow controller **forces** clarification before any other phase (when the triage classifier routes through the clarifier), so that I detect regressions where the supervisor skips the clarifier without authorization. The triage-skip path (where the classifier short-circuits the clarifier for a self-contained prompt) is an accepted alternative and must not fail the test.
...
2. The transcript contains the canonical required-subagent markers in the controller's feedback envelope (`requiredSubagent=product-generator` and `requiredSubagent=review-agent`) and asserts their order by first-occurrence index. The `requiredSubagent=clarifier` marker is conditional: when the triage classifier routes through the clarifier it must appear before the others; when triage skips (synthetic `ready_to_proceed` with `skipReason: "triage_classifier"`), the clarifier marker is absent and the test accepts that path. This proves the controller forced the canonical sequence (or its triage-shortcut equivalent) rather than letting the supervisor finalize early.
...
- As of the triage-gated-clarification change, the same narrow prompt is also the most likely trigger of the triage-skip path. The test accepts either path; what it pins is the downstream ordering invariant (product-generator before review-agent) and the absence of `phase=error`.
