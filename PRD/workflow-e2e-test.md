...
2. As a core maintainer, I want the test to assert that the workflow controller **forces** clarification before any other phase, so that I detect regressions where the supervisor skips the clarifier.
...
2. The transcript contains each required-subagent marker in the controller's feedback envelope: `requiredSubagent=clarifier`, `requiredSubagent=product-generator`, and `requiredSubagent=review-agent`. Order is asserted by comparing the index of each marker's first occurrence. This proves the controller forced the canonical sequence rather than letting the supervisor finalize early.
